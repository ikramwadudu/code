...
/*=====================================================[ CONNECTING  ]==============================================================*/                  	    
	    	...
	    	success('2', 'OM RUSLAY')
	    	setTimeout( () => {
	    	console.log(color(`Si Rusli Berhasil Terhubung`, 'pink'))
	    	}, 1000)
	    	setTimeout( () => {
	    	console.log(color(`Jangan Lupa Donasi Ya Gan`, 'pink'))
	    	}, 2000)
			setTimeout( () => {
				console.log(color(`Follow Instagram @jencreacven`, 'pink'))
				}, 3000)    	    	     	
         	}) 
			
			// OFF
         	// nayla.sendMessage(`${setting.ownerNumber}@s.whatsapp.net`, `*Si Rusli Telah Online*\nKetik *${prefix}elit* Untuk Aktivasi`, MessageType.text)
...
/*=======================================================[ WELCOME ]==============================================================*/                  	    
			...
			teks = `@${num.split('@')[0]} Telah Keluar Dari Grup`
			let buff = await getBuffer(ppimg)
			nayla.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		    } catch (e) {
...
*/                                        
                    ...
                    reply('BOT SEDANG OTW KE GRUP')                     
					break  
					case 'claim':
					if (!isGroup) return reply(`GRUP ONLY`)
					if (!isBotx) return reply(`SI Rusli TIDAK AKTIF\nSILAHKAN KETIK ${prefix}botx`)
			        ...
			        inf1 += `*BANG MINTA UANG BANG, KETIK ${prefix}donasi*`
			        reply(inf1)
			        break
					...
					console.log(color(`*SELAMAT ${pushname},KAMU TELAH RESMI MENJADI ELITE MEMBER*`, 'pink'))
					} else {
					addEliteUser(sender, pushname, time, serialUser)
					try {						 
					} catch {						  
                    ...
                    anu1 += `➸ *UKURAN* : ${anu.size}\n`
                    anu1 += `➸ *SUMBER* : ${anu.source}\n`
			        ...
			        if (isLimit(sender)) return
			        await limitAdd(sender)					 
					if (args.length < 1) return reply('Harap Masukan Link Youtube Nya')
					...
					anu3 = await getBuffer(anu.url)
					nayla.sendMessage(from, anu2, image, { quoted: nay, caption: anu1 })
					nayla.sendMessage(from, anu3, audio, { mimetype: 'audio/ogg', quoted: nay1 })
					break
				    case 'ytmp4':  
				    if (!isPrem) return reply(nyz.prem1(command))                    	  
                    if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
			        ...
			        await limitAdd(sender)				 
					if (args.length < 1) return reply('Harap Masukan Link Youtube Nya')
					if (!isUrl(args[0]) && !args[0].includes('youtube')) return reply('Link Nya Tidak Valid Kak')
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	                                          
			       ...
			       await limitAdd(sender)	 
                   reply(naylachan) 
                   anu = await fetchJson(`https://animechan.vercel.app/api/random`)
                   anu1 = `➻ *JUDUL* : ${anu.anime}\n`
                   anu1 += `➻ *CHARACTER* : ${anu.character}\n`
                   anu1 += `➻ *QUOTE* : ${anu.quote}\n`
                   reply(anu1)
                   break
                   case 'samehadaku':  
                   if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	                                          
				   ...
				   // Bagian Al - Quran (UPDATE 1 NOV 2021)
				   // ONGOING : 

				   //    1. Apabila User Memasukan Angka Mulai Lebih Besar Daripada Angka Akhir	// FIXED
				//    2. Urutan Surat Berdasarkan Nomor Bukan Index Array						// FIXED
				   // Tahap Pengembangan Ayat Array

				   // Niat Penambahan : Memilih Berapa Menu Yang Ditampilkan
				   ...
				   let surah = body.trim().split(/ +/).slice(1)
				   let awe = surah;
				   ...
				   if(surah.length > 2 ){
					   return reply(`Maaf Format Atau Inputan Anda Salah\n Contoh :\n*#quran 1/3*`)
				   }else{
					   let batas = awe.toString().search("/")
					   awe = awe.toString()
					   for(let i = 0 ; i<batas;i++){
						suratmulai += awe[i];
					  }

					  for(let i = batas+1 ; i < awe.length ; i++){
						suratakhir += awe[i]
					  }
					  if(Number(suratmulai) > Number(suratakhir)){
						  return reply("Maaf Inputan Anda Salah\n*_Surat Mulai Harus Lebih Kecil\nDari Surat Akhir_*\nContoh :\n*#quran 1/3*");
					  }
				   }
				...
				// 		   suratmulai = i;
				// 	   }
				// 	   if( surah[2] + surah[3] + surah[5] == ){
				// 		   suratakhir = i;
				// 	   }
				//    }

                   anu = await fetchJson(`https://equran.id/api/surat`)
				   if (isGroup) return reply(`Maaf Tampilannya Akan Banyak *Indikasi Spam*\nJadi Lewat Chat Pribadi Saja`)

				   anu1 = "";

				   for(let i = Number(suratmulai)-1; i <= Number(suratakhir)-1; i++){
					 anu1 += `➻ *Nomor* : ${anu[i].nomor}\n`
					 anu1 += `➻ *Nama Surat* : ${anu[i].nama} (${anu[i].nama_latin})\n`
					 anu1 += `➻ *Arti* : ${anu[i].arti}\n`
					 anu1 += `➻ *Turun* : ${anu[i].tempat_turun}\n`
					 anu1 += `➻ *Ayat* : ${anu[i].jumlah_ayat}\n\n`
				   }
					reply(`${anu1}`)
					// reply(`${surah.length}\n${surah[1]}`)
                	//  console.log(anu1)
				   ...
				   case 'kandungansurat' :
					...
					let surah1 = body.trim().split(/ +/).slice(1);
					let awe1 = surah1;

					let apiAlquran = await fetchJson('http://localhost/api/alquran.json');

					let ada1 = true;
					if(awe1.length < 2 && awe1.length > 0){
						reply(`Sedang Mencari ${awe1[0]}...`);
						for(let i = 0; i < 114 ;i++){
							if(apiAlquran[i].nama_latin == awe1[0]){
								reply('*Surat Yang Kamu Cari Ketemu*');
								reply(`${awe1}\n${apiAlquran[i].deskripsi}`)
								ada1 = false;
								break;
							}
							// else{
							// 	reply("MAAF SURAT YANG KAMU CARI TIDAK KETEMU !!")
							// }
						}
					}else{
						return reply(`*Format Anda Salah*\nContoh : #kandungansurat Al-Fatihah`);
					}

					if(ada1 == true){
						return reply(`Maaf Sepertinya Tidak Ketemu`);
					}
					break;
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	                                          
			       ...
			       await limitAdd(sender)	
				   reply('Tobat Goblok');			 
				//    qute = fs.readFileSync('menu/undef1.jpeg') 
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	                                          
			        ...
			        if (isLimit(sender)) return
			        await limitAdd(sender)
					if(!isQuotedSticker) return reply(`Permintaan Kamu Tidak Dapat Dilakukan atau Format Salah\n*Reply Stikernya Dulu*`);
					reply(naylachan)
			    	...
			    	boij = JSON.parse(JSON.stringify(nay).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
			     	delb = await nayla.downloadMediaMessage(boij)
			       	setiker.push(`${svst}`)
			    	fs.writeFileSync(`./src/stickers/`, delb)
			        ...
			        if (!isOwner) return reply(`*MAAF ANDA BUKAN OWNER BOT INI*`)
                    prefix = args[0]
			        ...
			        await limitAdd(sender)
				    if (args.length < 1) return
			        if (!isOwner) return reply(`*_Anda Bukan Pemilik Si Rusli_*`)
			        ...
			        if (isLimit(sender)) return
			        await limitAdd(sender)
					nayla.sendMessage(from, 'INGFO KONTAK OWNER\nPERCUMA DI CHAT',MessageType.text, { quoted: nay} )
					...
					.on('end', function () {
					console.log('FOLLOW INSTAGRAM @jencreacven')
					...
					for (let _ of anu) {
					nayla.sendMessage(_.jid, buff, image, {caption: `[ *IKLAN* \n\n${body.slice(4)}`})
					}
					reply('*IKLAN TERPASANG* ')
					} else {
					for (let _ of anu) {
					sendMess(_.jid, `*IKLAN*\n\n${body.slice(4)}`)
					}
					reply('*IKLAN TERPASANG* ')
					}
					break
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	 
                    ...
                    nayla.sendMessage(`0895@s.whatsapp.net`, optionsp, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "caption": `*NIHHH WOIII REQUEST*`} } } })                                    
                    ...
                    nayla.sendMessage(from, anu, image, {caption: `Ini Gan Hasilnya`, quoted: nay})
                    break
                    case 'nulis2':  
                    if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
			        if (isLimit(sender)) return
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	 
			        ...
			        if (isLimit(sender)) return
			        await limitAdd(sender)
					if (!isGroup) return reply(`*Berfungsi Pada Grup*`)
					if (!isGroupAdmins) return reply(`*_Maaf Anda Bukan Admin_*`)
					if (args.length < 1) return reply('PILIH 1/0')
					if (Number(args[0]) === 1) {
					if (isWelkom) return reply('*_FITUR TELAH AKTIF_*')
					welkom.push(from)
					fs.writeFileSync('./nayla/welkom.json', JSON.stringify(welkom))
					reply('*WELCOME TELAH _AKTIF_ / ~NONAKTIF~*')
					} else if (Number(args[0]) === 0) {
					welkom.splice(from, 1)
				    fs.writeFileSync('./f/welkom.json', JSON.stringify(welkom))
				    reply('*WELCOME TELAH ~AKTIF~ / _NONAKTIF_*')
					} else {
					reply(`......`)
					}
					break  
...
/*====================================================[ CASE LOLTEAM ]==============================================================*/                    	 
			        ...
			        if (isLimit(sender)) return
			        await limitAdd(sender) 
					if (!isOwner) return reply(`*MAAF ANDA BUKAN OWNER BOT*`)
					delp = body.slice(10)
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	 
                    ...
                    break
                    case 'dadu':
					reply("Sedang Dalam Penempaan Om")
                    // if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
			        // if (isLimit(sender)) return
			        // await limitAdd(sender)
